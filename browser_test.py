...
def llama_v2_prompt(
    ...
    B_SYS, E_SYS = "<<SYS>>\n", "\n<</SYS>>\n\n"
    BOS, EOS = "<s>", "</s>"
    DEFAULT_SYSTEM_PROMPT = f"""You are an agent controlling a browser. You are given:
...
You can issue these commands:
	...
	TYPE SUBMIT X "TEXT" - same as TYPE above, except then it presses ENTER to submit the form
...
like "TYPE SUBMIT 7 "search query"" to get to a more useful page.

Then, if you find yourself on a Google search results page, you might issue the command "CLICK 24" to click
on the first link in the search results. (If your previous command was a TYPE SUBMIT your next command should
probably be a CLICK.)
...
if __name__ == '__main__':
    llm = Llama(model_path="./llama-2-7b-chat.ggmlv3.q5_1.bin", n_ctx=4096, use_mlock=True, f16_kv=True, n_threads=8)
    _crawler = Crawler()
    ...
    def get_gpt_command(objective, url, previous_command, browser_content):
        messages_list = [
            {
                'role': 'user',
                'content': """CURRENT BROWSER CONTENT:
------------------
...
"""
            ...
            {
                'role': 'assistant',
                'content': 'TYPE SUBMIT 8 "dorsia nyc opentable"'
            },
#             {
#                 'role': 'user',
#                 'content': """CURRENT BROWSER CONTENT:
...
#             {
#                 'role': 'assistant',
#                 'content': 'TYPE SUBMIT 8 "anchorage redfin"'
#             },
        ]
        messages_list.append({
            'role': 'user',
            'content': f"""CURRENT BROWSER CONTENT:
------------------
...
"""
        ...
        prompt = llama_v2_prompt(messages_list)
        response = llm(prompt, temperature=.5)
        with open('browser_debug.log', 'w') as fp:
            fp.write(prompt + '\n\n')
    ...
    objective = "Buy me an used office chair"
    ...
    try:
        while True:
            ...
            print("Previous command: " + prev_cmd)
            print("----------------\n" + browser_content + "\n----------------\n")
            if len(gpt_cmd) > 0:
